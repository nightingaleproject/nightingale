...
namespace :nightingale do
  namespace :demo do
    ...
    # Helper method: Given a death record with data in Nightingale format create the record
    def create_record(record_data)
      ...
      DeathRecord.transaction do
        ...
        death_record.save
        steps_content_hash = death_record.separate_step_contents(record_data)
    ...
    desc "Load FHIR records from a supplied directory"
    # Run it as rails nightingale:demo:load_fhir_records RECORDS="`*.json`" (yuck, sorry)
    task load_fhir_records: :environment do
      raise 'Please provide the records to load, e.g., RECORDS="`echo *.json`"' unless ENV['RECORDS']
      # Load the files provided
      ENV['RECORDS'].split(/\s/).each do |filename|
        puts "Loading #{filename}"
        file = File.read(filename)
        # Use the translation service
        response = RestClient.post "http://localhost:8080/nightingale", file, { content_type: 'application/fhir+json' }
        record_data = JSON.parse(response.body)
        create_record(record_data)
      end
    end

    desc "Poll API client for responses"
    task poll_client: :environment do
      # Regularly ping the API Client to see if we have any responses
      # Note: We'll eventually put this in a loop, in which case this task won't terminate on its own
      while true do
        sleep 30
        # See if there are any records where we're waiting for information; if we don't have a cause code that
        # means we're waiting for at least something (might be an ACK, might be coding, etc)
        # TODO: This ignores race and ethnicity coding
        # NOTE: This model feels odd, where we *HAVE* to track what we want a response for rather than just processing all responses
        # TODO: It might even be more than odd, in that if NCHS recodes something (do they do that?) we won't see it this way
        records = DeathRecord.where(currently_submitted: true, underlying_cause_code: nil)
        puts "Looking for info on #{records.count} records: #{records.map(&:id).join(', ')}"
        records.each do |record|
          deathYear = Date.parse(record.contents['dateOfDeath.dateOfDeath']).year
          jurisdictionID = record.contents['deathLocationJurisdiction.deathLocationJurisdiction']
          certificateNumber = "%06d" % record.contents['certificateNumber.certificateNumber']
          url = "http://localhost:4300/record/status/#{deathYear}/#{jurisdictionID}/#{certificateNumber}"
          puts "Checking status on record #{deathYear}#{jurisdictionID}#{certificateNumber} (#{url})"
          response = RestClient.get(url)
          if response.code == 200
            response = JSON.parse(response.body)
            case response['message']['status']
            when 'Pending'
              puts "  Status is still pending"
            when 'Sent'
              puts "  Status is still sent (#{response['message']['status']} retries)"
            when 'Acknowledged'
              if record.acknowledgement_message_id
                puts "  No change in acknowledgement status"
              else
                puts "  Message was acknowledged"
                # TODO: We don't see the ACK itself (and likely don't need to anymore), create a fake ack ID until we update Nightingale
                record.update_attributes(acknowledgement_message_id: "ACK-#{Time.now.to_i}")
              end
            when 'AcknowledgedAndCoded'
              # Note the ACK part in case it wasn't seen before
              if record.acknowledgement_message_id
                puts "  No change in acknowledgement status"
              else
                puts "  Message was acknowledged"
              ...
              if record.coding_message_id
                puts "  No change in coding status"
              else
                puts "  Message was coded"
                # TODO: we need to make the coding data available in a standard way, for now just note that we have the coding info
                record.update_attributes(coding_message_id: "CODING-#{Time.now.to_i}", underlying_cause_code: "ICD")
              end
            end
          else
            puts "Error retrieving data from Reference Client"
          end
        end
      end
    end
