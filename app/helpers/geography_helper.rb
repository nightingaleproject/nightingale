...
module GeographyHelper
  def self.get_states
    states = Geography::State.pluck(:name)
    states = [] unless states
    states.sort_by! { |e| ActiveSupport::Inflector.transliterate(e.downcase) }
  end

  def self.get_counties(state)
    state_obj = Geography::State.find_by(name: state)
    counties = Geography::County.where(state: state_obj).pluck(:name) if state_obj
  ...
  def self.get_cities(state, county)
    ...
    cities = Geography::City.where(state: state_obj, county: county_obj).pluck(:name) if county_obj
  ...
  def self.get_cities_in_state(state)
    state_obj = Geography::State.find_by(name: state)
    cities = Geography::City.where(state: state_obj).pluck(:name) if state_obj
    cities = [] unless cities
    cities.sort_by! { |e| ActiveSupport::Inflector.transliterate(e.downcase) }
  end
  ...
  def self.get_zipcodes(state, county, city)
    ...
    zipcodes = Geography::Zipcode.where(state: state_obj, county: county_obj, city: city_obj).pluck(:name)
