...
module IJEFormat
  ...
  def self.obtain_ssn(record_hash)
    if !record_hash.key?("ssn.ssn1") || !record_hash.key?("ssn.ssn2") || !record_hash.key?("ssn.ssn3")
      return ""
    else
  ...
  def self.pregnancy_lookup(record_hash, key)
    ...
    {
      "Not pregnant within past year" => "1",
      "Pregnant at time of death" => "2",
  ...
  def self.certifier_lookup(record_hash, key, offset)
    ...
    certifier_hash =
    {
      "Physician (Certifier)" => "D",
      "Physician (Pronouncer and Certifier)" => "P",
      "Medical Examiner" => "M",
      "Coroner" => "C",
    }
  ...
  def self.death_occurrence_lookup(record_hash, key)
    ...
    {
      "Death in hospital" => "1",
      "Death in hospital-based emergency department or outpatient department" => "2",
      "Dead on arrival at hospital" => "3",
      "Death in home" => "4",
      "Death in hospice" => "5",
      "Death in nursing home or long term care facility" => "6"
    }
  ...
  def self.education_lookup(record_hash, key)
    ...
    {
      "8th grade or less" => "1",
      "9th through 12th grade; no diploma" => "2",
      "High School Graduate or GED Completed" => "3",
      "Some college credit, but no degree" => "4",
      "Associate Degree" => "5",
      "Bachelor's Degree" => "6",
      "Master's Degree" => "7",
      "Doctorate Degree or Professional Degree" => "8",
      "Unknown" => "9",
    }
  ...
  def self.process_data(records)
    ije_records = []
    records.each do |record_obj|
      record = record_obj.contents
      record_ije = IJE::MortalityFormat.new(date_of_death_year: IJEFormat.extract_date(record, "dateOfDeath.dateOfDeath", "year"),
                                            ...
                                            sex_edit_flag: 0,
                                            social_security_number: IJEFormat.obtain_ssn(record),
                                            decedents_age_type: "",
                                            decedents_age_units: "",
                                            ...
                                            decedents_residence_city: IJEFormat.lookup(record, "decedentAddress.zip", 5),
                                            ...
                                            decedents_race_black_or_african_american: IJEFormat.race_lookup(record, "race.race.specify", "Black or African American"),
                                            decedents_race_american_indian_or_alaska_native: IJEFormat.race_lookup(record, "race.race.specify", "American Indian or Alaskan Native"),
                                            ...
                                            decedents_race_guamanian_or_chamorro: IJEFormat.race_lookup(record, "race.race.specify", "Guamanian") || IJEFormat.race_lookup(record, "race.race.specify", "Chamorro"),
                                            decedents_race_samoan: IJEFormat.race_lookup(record, "race.race.specify", "Samoan"),
                                            decedents_race_other_pacific_islander: IJEFormat.race_lookup(record, "race.race.specify", "Other Pacific Islander"),
                                            ...
                                            date_of_injury_year: "",
                                            time_of_injury: "",
                                            injury_at_work: IJEFormat.lookup(record, "deathResultedFromInjuryAtWork.deathResultedFromInjuryAtWork", 1),
                                            ...
                                            place_of_death_zip_code: record["locationOfDeath.zip"],
                                            ...
                                            decedents_residence_unit_or_apt_number: record["decedentAddress.apt"]&.truncate(7),
                                            decedents_residence_city_or_town_name: IJEFormat.lookup(record, "decedentAddress.city", 28),
                                            decedents_residence_zip_code: record["decedentAddress.zip"],
                                            ...
                                            was_case_referred_to_medical_examiner_coroner: IJEFormat.lookup(record, "meOrCoronerContacted.meOrCoronerContacted", 1),
                                            ...
                                            certifier_unit_or_apt_number: record["personCompletingCauseOfDeathAddress.apt"]&.truncate(7),
                                            ...
                                            certifier_zip: record["personCompletingCauseOfDeathAddress.zip"],
                                            certifier_date_signed: IJEFormat.extract_date(record, "dateCertified.dateCertified", "format"),
                                            date_filed: "",
                                            state_us_territory_or_canadian_province_of_injury_literal: IJEFormat.territory_lookup(record, "placeOfInjury.state", "placeOfInjury.city", "placeOfInjury.country", true),
      ...
      ije_records.push(record_ije)
    end
    return IJE::MortalityFormat.write(ije_records)
  end
end
